...
**当前版本：v1.17.0**（线上版本可通过 `GET /healthz` 的 `version` 字段确认）

无需登录抖音账号、基础解析无需注册本站账号的抖音视频 / 图集下载工具。粘贴分享链接，在线预览并下载无水印原片。内置 **AnyToCopy 统一解析 + 代理 IP 池 + 管理后台 + 可选用户体系 + 按次计费开放 API**。
...
- 🖼 **图集下载**：逐张原图浏览器直连下载
- ⚡ **统一视频解析 API**：网页解析、开放 API 和分享页 worker 全部调用 AnyToCopy `/video/extract`；普通解析只提交 `workUrl`，不传 `taskType`，默认不获取语音文案
...
从整段分享文案提取 `v.douyin.com` 短链 → `POST /video/extract`（普通解析只传 `workUrl`）→ 每 4 秒 `GET /video/query` → 把返回的视频/图集地址与作品元数据归一化为站内结果。
