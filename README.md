...
**当前版本：v1.18.0**（线上版本可通过 `GET /healthz` 的 `version` 字段确认）

支持抖音、小红书、快手、B站、微博、视频号、Twitter/X、TikTok、YouTube 等 **50+ 内容平台**的视频 / 图集解析工具。无需登录源平台，基础解析也无需注册本站账号。内置 **AnyToCopy 统一解析 + 代理 IP 池 + 管理后台 + 可选用户体系 + 按次计费开放 API**。

## 功能

- 🎬 **多平台原片解析**：AnyToCopy 返回无水印视频、图集与作品信息；已验证的同源视频线路使用 1 字节 Range 预检后原生流式保存，其他平台提供原片直达入口
- 🔗 **分享页**：抖音链接发微信打不开？一键生成 `/s/{短码}` 分享页，**微信里点开就能看**，不用装 App；`POST /api/shares` 只校验链接就立即返回本地分享地址，抖音内容由可恢复的持久化 worker 异步获取
- 🖼 **图集下载**：逐张原图浏览器直连下载
- ⚡ **50+ 平台统一解析 API**：网页解析与开放 API 接受受支持平台的 HTTPS 作品链接，统一调用 AnyToCopy `/video/extract`；普通解析只提交 `workUrl`，不传 `taskType`，默认不获取语音文案
- ▶️ **播放直连、下载同源**：新结果优先播放 AnyToCopy 返回的媒体直链，失败自动落到受签名保护的 `/api/atc/video/{item_id}`；旧分享页继续兼容 `/api/video/{vid}`，服务器只流式转发、不落地媒体
- 📋 **粘贴即用**：直接粘贴整段分享文案，自动识别受支持的平台链接；支持多平台混合批量解析、结果导出 Excel
- ✨ **Apple 风格交互**：桌面端以原生粘性滚动呈现「粘贴 → API 解析 → 原片/分享」流程，输入识别、结果出现与主题切换均有即时反馈；移动端和“减少动态效果”偏好自动使用轻量静态布局
...
首次启动后打开 `http://127.0.0.1:3344/admin_d`，在「视频解析 API」中填写 AnyToCopy API Key / Secret 并保持主解析开启。未配置有效密钥时，网页、开放 API 与分享页解析都会明确返回 `503`；普通解析默认不会请求语音文案。
...
AnyToCopy API Key/Secret、API 单价与微信公众号 AppID/Secret 不走环境变量，在管理后台里配置（存数据库）。

### 部署注意事项

- **必须单 worker 运行**（默认即是）。会话、验证码、限频计数都在进程内存里，多 worker 互不相认会导致登录/验证码随机失效。也因此**重启会掉登录态**，属预期行为。
- **上线后第一件事**：打开 `/admin_d` 登录，在「视频解析 API」配置有效的 AnyToCopy Key/Secret，再在「代理」里添加代理 IP。元数据解析直连 AnyToCopy；同源媒体流与旧兼容线路仍按代理策略访问抖音 CDN。
...
从整段分享文案中识别抖音、小红书、快手、B站、微博、视频号、Twitter/X、TikTok、YouTube 等受支持的 HTTPS 作品链接 → `POST /video/extract`（普通解析只传 `workUrl`）→ 每 4 秒 `GET /video/query` → 把返回的视频/图集地址与作品元数据归一化为站内结果。

**分工**：作品信息与无水印地址统一由 AnyToCopy API 提取；浏览器先直连其返回的媒体地址。视频下载和直连失败后的播放使用带 `Content-Disposition` 的 `/api/atc/video/{item_id}?exp=...&sig=...` 同源流：前端先发 1 字节 Range 验证线路，再交给浏览器原生下载，不把完整视频堆进页面内存。该路由只按 `item_id` 读取服务端缓存，不接受任意 URL，且会再次校验抖音媒体域名；媒体授权带 HMAC 有效期签名、单 IP 限频与并发限制，只支持单段 Range。旧分享数据继续兼容 `/api/video/{vid}`。每条播放线路的成败都会进入后台「播放诊断」看板。
...
- **直连会向媒体源发出请求**：浏览器直连播放视频或加载/下载图片时，对应媒体主机会接收到请求方的 IP 等网络信息与浏览器信息；已验证的同源兼容线路由本站服务器流式转发。
- **解析链接会提交给 AnyToCopy**：网页、开放 API 与分享页解析都会把规范化后的作品链接提交给 AnyToCopy，以获取作品元数据和媒体地址。普通解析不传 `taskType`、不请求语音文案；只有用户主动打开「获取文案」且管理员启用该功能时，才额外提交 `taskType=TEXT`。本站不保存媒体文件。
...
| v1.17.0 | 2026-08-28 | 首页新增 Apple 风格的原生滚动叙事，以单一粘性舞台演示「粘贴链接 → AnyToCopy API 解析 → 原片下载/分享」流程，并支持步骤回看；解析器新增链接识别、粘贴确认、真实加载扫描、成功/错误状态与结果连续出现动画，明暗主题使用圆形扩散过渡，导航滚动后转为轻量悬浮。动效只使用 CSS、Web Animations API、IntersectionObserver 和单个 `requestAnimationFrame`，不劫持滚动、不引入 GSAP/WebGL；移动端取消粘性舞台，`prefers-reduced-motion` 下关闭非必要动效并展示完整静态内容。同步移除旧首屏装饰网格、弹跳缓动和彩色按钮光晕。 |
| v1.16.0 | 2026-08-28 | 参考 AnyToCopy 的信息层级重构首页：移除大面积极光与多色渐变，改为克制的网格背景、单色重点和更紧凑的首屏；输入区升级为清晰的链接输入、独立解析按钮、粘贴/文件上传入口与视频/图集/批量能力提示，移动端改为纵向主操作并收敛导航；同步精简信任卡片、中英文文案与加载状态，保留明暗主题和全部原有解析、批量、分享功能。 |
| v1.15.0 | 2026-08-28 | 网页解析、开放 API 与分享页 worker 全量切换为 AnyToCopy `/video/extract` + `/video/query`；普通解析仅传 `workUrl`，默认不获取语音文案，主动文案任务才传 `taskType=TEXT`。新增受签名保护的 `/api/atc/video/{item_id}` 播放/下载流与 `work_url` 缓存，兼容临时媒体地址刷新、Range/断点续传及旧 `/api/video` 分享数据；后台「增强线路」重构为「视频解析 API」，主解析默认开启、文案默认关闭，并同步隐私说明、测试工具与前端线路。 |
