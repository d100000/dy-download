...
**当前版本：v1.24.2**（线上版本可通过 `GET /healthz` 的 `version` 字段确认）
...
- 🔄 **前端版本缓存**：所有页面注入应用版本与 UTC 构建时间戳，并通过 `X-Frontend-Version` 响应头提供检查入口；本地图标附带 `?v=` 版本参数。页面禁止持久缓存，当前版本静态资源长期缓存，旧版或无版本请求重新校验。
