...
**当前版本：v1.21.3**（线上版本可通过 `GET /healthz` 的 `version` 字段确认）
...
- 🎬 **50+ 平台原片解析**：统一提取视频、图集、标题、作者、封面、时长与分辨率；首页结果按实际数据展示点赞、评论、分享和收藏数，缺失项自动隐藏，有效的 0 正常显示
- 🔗 **分享页**：抖音链接发微信打不开？一键生成 `/s/{短码}` 分享页，**微信里点开就能看**，不用装 App；`POST /api/shares` 只校验链接就立即返回本地分享地址，后台 worker 再按平台异步获取内容
- 🖼 **图集下载**：抖音图片通过受 HMAC 保护的同源端点按作品 ID 惰性刷新，其他平台逐张下载；数据库不保存短时签名地址
- ⚡ **主服务优先、缺失补全**：所有平台优先调用后台配置的解析服务；抖音缺少媒体、作者或互动数据时通过官方接口补全，主服务不可用时自动兜底。其他平台保持主服务解析。
- ▶️ **播放直连、下载同源**：主服务媒体使用受 HMAC 保护的 `/api/media/video/{item_id}`，抖音官方补充媒体使用 `/api/douyin/video/{item_id}`；旧媒体路由保持兼容。主媒体过期或失败时，抖音可按保存的来源补充。
- ⏱ **真实媒体时长**：解析结果未返回时长或分辨率时，首页通过视频元数据自动补全；读取前与读取失败均显示明确状态，不再用 `0:00` 或 `720P` 冒充真实数据
- 📋 **粘贴即用**：直接粘贴整段分享文案，自动识别受支持的平台链接；支持多平台混合批量解析、结果导出 Excel
- ✨ **简洁首页**：以链接输入和解析按钮为主，支持平台与常见问题按需展开；保留批量解析、下载、分享、明暗主题与中英双语。
- 👤 **可选账号 + 免费配额**：匿名/登录用户每日免费额度采用原子预占防并发超用；注册、登录使用滑块验证码防机器人
- 📝 **文案提取（语音转文字，主动开启）**：该开关默认关闭；只有用户主动获取文案时才额外提交 `taskType=TEXT`，注册用户默认每天 5 次（原子预占/失败退款，同一视频全站缓存共享）
...
ADMIN_PASSWORD=a-strong-random-password ./run.sh
HOST=0.0.0.0 ADMIN_PASSWORD=a-strong-random-password ./run.sh  # 明确允许局域网访问
```

首次启动后打开 `http://127.0.0.1:3344/admin_d`，在「视频解析 API」保存有效的 API Key / Secret 并启用主服务。抖音官方接口用于缺失补全和失败兜底；完整媒体兜底需要 Chromium。普通解析不会请求语音文案。

手动方式：

```bash
pip install -r requirements.txt
REQUIRE_ADMIN_PASSWORD=1 ADMIN_PASSWORD=a-strong-random-password \
  uvicorn server:app --host 0.0.0.0 --port 3344 --no-access-log
```
...
| 变量 | 默认 | 说明 |
|---|---|---|
| `ADMIN_PASSWORD` | 本地模式为 `douyin-admin`；容器必填 | 管理后台密码；生产建议至少 12 位随机值 |
| `REQUIRE_ADMIN_PASSWORD` | 本地关 / Docker 开 | 设 `1` 后密码缺失、过短或为默认值时 fail-closed |
...
| `NEW_KEY_BALANCE` | `100` | 每个用户仅首个 API Key 领取的试用余额（单位：分） |
...
| `IMAGE_REQUESTS_PER_MIN` | `240` | 单 IP 每分钟同源图片请求上限，可覆盖 100 张展示 + 逐张下载 |
| `IMAGE_MAX_BYTES` | `52428800` | 同源图集端点允许的单图最大字节数（1–100 MiB） |
...
| `PARSE_TEXT_MAX` / `BATCH_TEXT_MAX` | `8192` / `65536` | 单条/批量解析文本的字符上限 |
| `ATC_WORKERS` | `1` | 兼容解析后台队列 worker 数，上限受在途任务数限制 |
...
解析服务凭据、API 单价和微信公众号密钥均在管理后台配置（存数据库）。接口基址固定由服务端管理，不在页面、状态响应或报错中暴露；普通解析只请求媒体与元数据，主动文案功能才额外请求转录。

### 部署注意事项

- **必须单 worker 运行**（默认即是）。会话、验证码、限频计数都在进程内存里，多 worker 互不相认会导致登录/验证码随机失效。也因此**重启会掉登录态**，属预期行为。
- **上线后第一件事**：在 `/admin_d` 的「视频解析 API」中配置并测试主服务凭据；抖音补充链路需可用的 Chromium 与网络出口。启用强制代理时，需先添加并测试代理。
...
- **服务器不落地任何媒体**：只存必要元数据、`item_id` 与短时媒体缓存；签名地址过期时按作品 ID 和受保护的原始链接重新排队刷新
- **播放线路统一可恢复**：主媒体使用 `/api/media/video/{item_id}`，抖音补充媒体使用 `/api/douyin/video/{item_id}`；历史路由继续兼容。播放尝试和结果进入后台播放诊断。
...
从整段分享文案中识别公开 HTTPS 作品链接，统一优先调用主解析服务。抖音缺少媒体、作者、互动数、封面、时长或尺寸时，通过服务器官方接口补全；已有字段不被覆盖。补充接口只有元数据时，也可补全主服务已返回的媒体结果。主服务失败则尝试完整官方链路；两条路径都没有媒体时返回可重试错误。普通解析不请求或保存语音文案。

**分工**：主媒体通过 `/api/media/video/{item_id}?exp=...&sig=...`，官方补充媒体通过 `/api/douyin/video/{item_id}`；均校验 HMAC、媒体域名、内容类型、Range 与并发限制。官方签名媒体仅保留在内存，分享快照移除短时地址。前端先发 1 字节 Range 预检，再交给浏览器原生下载。新响应使用中性 `source: parser` 和 `direct_url`；历史路由仍兼容，数据库中的内部名称不迁移。

## 隐私与数据边界

- **不保存媒体文件**：服务器不落地保存视频或图片；官方/兼容线路仅流式转发媒体字节。分享页会按有效期保存标题、封面、作者、作品 ID 等必要元数据。
- **站内账号可选**：基础解析无需注册，也无需登录源平台。注册本站账号时会保存邮箱与加盐密码哈希；账号资料随账号保留。
...
- **直连会向媒体源发出请求**：浏览器直连播放视频或加载/下载图片时，对应媒体主机会接收到请求方的 IP 等网络信息与浏览器信息；视频下载和播放兜底可经本站受保护的同源流式端点转发。
- **解析链路与数据最小化**：公开作品链接会提交给已配置的第三方内容解析服务；抖音缺失信息由服务器通过官方接口补全。普通解析默认不获取语音文案，只有主动开启文案功能才额外提交转录任务。本站不保存媒体文件。

## 更新日志

| 版本 | 日期 | 内容 |
|---|---|---|
| v1.21.3 | 2026-09-09 | 优化互动数据展示：统一浅底分栏、突出数字并将标签置于下方；根据结果区域宽度自动切换四列或两列，缺失项自适应排列，批量列表保留紧凑样式。 |
| v1.21.2 | 2026-09-09 | 首页视频、图集、批量列表及预览增加紧凑的互动数据行，按字段显示点赞、评论、分享与收藏；缺失或无效数据自动隐藏，有效的 0 正常展示，中英文保持一致。 |
| v1.21.1 | 2026-09-08 | 精简首页：移除滚动演示、卖点卡片与 API 宣传区，缩短首屏标题和重复提示；平台列表与常见问题默认折叠，清理相关样式、动效与翻译，保留解析、批量、下载和分享功能。 |
| v1.21.0 | 2026-09-08 | 主解析服务优先；抖音缺失视频、作者、互动数及媒体元数据由官方接口补全；保留已有有效字段与统计精度。统一中性错误，清理后台服务商名称、域名和历史原始错误；新增中性媒体、文案及管理接口并兼容旧路由；补齐解析、作者与错误边界回归。 |
| v1.20.8 | 2026-08-31 | 一次性收口安全与可靠性审查：修复图集分享空视频崩溃及过期签名地址，新增按作品 ID 刷新的同源图片端点；ATC 响应统一解包，队列改为 SQLite 租约/CAS 且可有界停止；修复验证码非有限数绕过、API Key 试用金重复领取、限流内存增长、公开写接口滥用、Excel 公式注入、短链自循环、Range 416 误刷新、Chromium 锁/代理回退与缓存边界；移除图片内联事件注入面，补齐微信播放回退和剪贴板降级；Docker 默认要求强管理密码。 |
| v1.20.7 | 2026-08-29 | 修正 Chromium DevTools Protocol 的代理认证响应字段，带账号密码的 HTTP(S) 代理现在可以完成官方抖音详情与媒体链路认证，不再因认证挑战未继续而超时。 |
| v1.20.6 | 2026-08-29 | 官方抖音解析在 Chromium 代理场景补齐 HTTP(S) 认证挑战处理；Docker 镜像内置 Chromium，确保不依赖第三方解析 API 时仍可获取详情、作者、互动数据与媒体。 |
| v1.20.5 | 2026-08-29 | 修复官方抖音链路的嵌套状态/媒体结果归一化与未来时间缓存判断；分享页过期媒体改为非阻塞惰性刷新，旧快照不再误入兼容解析任务。 |
| v1.20.4 | 2026-08-29 | 抖音分享链接、数字作品 ID、视频播放/下载及作者富化统一收口到抖音官方网页/接口链路；不再把抖音播放任务送入第三方解析 API。分享快照不公开内部来源链接，官方签名地址按需刷新；补齐旧快照迁移、Range 流、作者/互动数据与移动端比例回归测试。 |
| v1.20.3 | 2026-08-29 | 收口官方解析兼容性：旧分享快照和缺失的兼容媒体缓存不再下发过期地址，统一标记媒体状态并惰性刷新；补齐多格式时长归一化，修正文档中的官方媒体 URL 示例。 |
| v1.20.2 | 2026-08-29 | 修复抖音官方链路：抖音短链和数字作品 ID 不再进入第三方解析 API，改为隔离 Chromium 捕获官方 detail JSON，返回作者与互动数据及官方签名媒体地址；新增 HMAC + Range 的 `/api/douyin/video/{item_id}` 同源播放/下载流，微信内优先该线路，CDP 事件队列与签名代理绑定问题一并修复。 |
| v1.20.1 | 2026-08-29 | 补齐分享页与后台可靠性：修复登录同步分享创建的 SQLite 锁重入死锁，分享快照不再持久化短时媒体地址，关闭后台 worker 时使用有界等待，并补齐移动端视频比例与元数据回归测试。 |
| v1.19.3 | 2026-08-29 | 修复登录用户创建同步分享页时的 SQLite 全局锁自死锁：分享有效期在事务锁外计算并复用已查询的用户状态，避免锁内再次进入 `db_exec`；新增登录分享回归测试，覆盖配额结算与非递归加锁。 |
| v1.19.2 | 2026-08-29 | 补齐分享页与首页的展示规则：移除分享卡片及分享海报中的点赞、评论、收藏、分享数据；结果徽章不再展示直连/兼容线路策略，页脚改用供应商中性的第三方处理告知，并移除公开模板注释中的供应商名称。保留 v1.13.2 的移动端竖屏视频按比例限宽与水平居中修复。 |
| v1.19.1 | 2026-08-29 | 首页对外文案改为供应商中性：移除输入区、平台说明、滚动演示、结果徽章、结果提示及中英文 SEO/FAQ 中的 内容解析服务 名称、接口路径与内部线路描述；公开解析错误同步改为稳定的通用提示。隐私说明仍明确披露作品链接会提交给第三方内容解析服务，但不公开具体供应商与后台实现。 |
| v1.19.0 | 2026-08-29 | 精简首页解析结果：移除视频与图集结果中的点赞、评论、收藏、分享数卡片，并移除批量表格的互动数据列。视频结果改为监听媒体链接的 `loadedmetadata` / `durationchange`，在 内容解析服务 未返回时自动补全真实时长和分辨率，批量预览也会回填对应行；读取中与失败状态明确显示，不再伪造 `0:00` / `720P`，并支持一小时以上时长格式。 |
| v1.18.0 | 2026-08-28 | 首页升级为 50+ 平台统一解析入口：输入框下方新增受支持平台模块与本地化 Logo，明确展示抖音、小红书、快手、B站、多多视频、Twitter/X、今日头条、视频号、微博、TikTok、YouTube 与更多平台；网页单条/批量解析和开放 API 链接识别同步放开为安全的已知平台 HTTPS 域名，结果卡显示来源平台并只对抖音展示专属分享页功能。同步更新中英文首页介绍、SEO/JSON-LD、隐私边界和回归测试。 |
| v1.17.0 | 2026-08-28 | 首页新增 Apple 风格的原生滚动叙事，以单一粘性舞台演示「粘贴链接 → 内容解析服务 API 解析 → 原片下载/分享」流程，并支持步骤回看；解析器新增链接识别、粘贴确认、真实加载扫描、成功/错误状态与结果连续出现动画，明暗主题使用圆形扩散过渡，导航滚动后转为轻量悬浮。动效只使用 CSS、Web Animations API、IntersectionObserver 和单个 `requestAnimationFrame`，不劫持滚动、不引入 GSAP/WebGL；移动端取消粘性舞台，`prefers-reduced-motion` 下关闭非必要动效并展示完整静态内容。同步移除旧首屏装饰网格、弹跳缓动和彩色按钮光晕。 |
| v1.16.0 | 2026-08-28 | 参考 内容解析服务 的信息层级重构首页：移除大面积极光与多色渐变，改为克制的网格背景、单色重点和更紧凑的首屏；输入区升级为清晰的链接输入、独立解析按钮、粘贴/文件上传入口与视频/图集/批量能力提示，移动端改为纵向主操作并收敛导航；同步精简信任卡片、中英文文案与加载状态，保留明暗主题和全部原有解析、批量、分享功能。 |
| v1.15.0 | 2026-08-28 | 网页解析、开放 API 与分享页 worker 全量切换为 内容解析服务 `/video/extract` + `/video/query`；普通解析仅传 `workUrl`，默认不获取语音文案，主动文案任务才传 `taskType=TEXT`。新增受签名保护的 `/api/atc/video/{item_id}` 播放/下载流与 `work_url` 缓存，兼容临时媒体地址刷新、Range/断点续传及旧 `/api/video` 分享数据；后台「增强线路」重构为「视频解析 API」，主解析默认开启、文案默认关闭，并同步隐私说明、测试工具与前端线路。 |
...
| v1.12.0 | 2026-08-07 | 新增可选的 内容解析服务 增强线路（后台「增强线路」页签配置密钥，默认关闭）：① 分享页播放地址增强——API 返回的无水印地址作为第四条播放线路，带有效期判断、过期自动落链并惰性重新获取，播放线路优先级可在后台拖拽排序（`share_play_priority`）；② 注册用户「文案提取（语音转文字）」——结果卡开关提交异步任务、前端轮询展示，独立每日限额（默认 5 次，原子预占/失败退款），同一视频全站缓存共享、命中不扣次。ATC 全部请求经 `atc_jobs` 队列表串行提交（对方并发上限 5），永不进入同步主解析，故障自动降级。首页 UI 精简：删除与 trust 卡重复的 6 个信任徽章与 3 个步骤条，输入区 4 按钮减为 2 个（清空改为浮动 ×、上传文件降为文字链接），结果卡删「新窗口播放」，批量行内删「播放」，登录后导航改为邮箱下拉菜单；修复 `.fallback-hint` 缺样式。注册引导：匿名点文案开关直接弹登录且成功后自动继续、429 配额用尽附登录链接、第 2 次解析成功后可关闭的轻提示；分享页 CTA 强化、海报页脚带域名、「我的分享」播放 ≥10 显示 🔥；滑块验证码通过/失败内存计数并入后台状态。新增 `/transcript` 文案提取落地页（独立 SEO + FAQ JSON-LD），首页 FAQ 与隐私披露同步第三方边界（开启增强线路才会向 内容解析服务 提交链接），sitemap/llms.txt 同步 |
