...
**当前版本：v1.27.0**（首页底部显示应用版本与前端时间戳；也可通过 `GET /healthz` 的 `version` 字段确认）
...
- 💰 **网页账户余额**：后台用户列表可增加或设置可用余额，并查看流水；每日免费次数用完后自动使用余额。网页解析与语音转文字默认各 0.03 元/次，可在用户页的“网页计费”中分别配置。成功结算、失败退款，批量与分享页解析同样适用。
...
管理后台 `/admin_d` → **用户**：上方设置网页解析与语音转文字价格（元/次，最多两位小数，0 表示免费）；列表展示每日免费剩余、账户可用余额、任务预留金额、网页消费及 API 消费。点击“编辑余额”可以增加余额或设置可用余额，填写备注，并查看最近 30 条流水。并发使用导致余额变化时，后台拒绝用旧数值覆盖新余额；网络重试不会重复增加金额。
...
- `GET/POST /api/admin/billing-prices`：管理员读取或设置网页解析、文案提取价格，字段为整数分 `parse_price_cents`、`transcript_price_cents`。
