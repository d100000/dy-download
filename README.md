...
**当前版本：v1.28.0**（首页底部显示应用版本与前端时间戳；也可通过 `GET /healthz` 的 `version` 字段确认）
...
- 💾 **完整信息快照**：解析成功后，作品信息、互动数及已获取的作者资料在 SQLite 暂存 24 小时；生成分享页优先复用快照，重启后无需重复解析或扣次。分享页按自身有效期独立保存快照，展示完整文案、点赞/评论/分享/收藏、媒体参数、作者资料、音乐和标签；缺失项隐藏、有效的 0 保留，注明数据获取时间。打开或轮询已完成分享页不调用解析服务，媒体地址仅在播放/下载时按需刷新。
